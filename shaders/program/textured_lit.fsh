...
void main() {
   ...
   #ifdef ENABLE_SHADOWS
      // حساب الظلال الديناميكية مع حركة الشمس
      float lightStrength = max(0.75*isLava, getLightStrengthWithShadows(feetPos, diffuse, shadowModelView, shadowProjection, shadowtex1));
   ...
   #else
      // إضاءة ثابتة في حالة عدم تفعيل الظلال
      ambient.rgb *= LIGHT_BRIGHTNESS;
   #endif
